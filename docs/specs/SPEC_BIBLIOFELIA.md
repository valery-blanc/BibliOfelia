...
- Filtres dans la page catalogue (`catalog:record_list`) : catégorie, type de document, langue, **emplacement** (FEAT-051 — sélecteur `Location` ; une notice est retenue si **au moins un** de ses exemplaires est dans l'emplacement choisi : `records.filter(items__location_id=location).distinct()`), **tag** (recherche substring case-insensitive sur le nom — `science` matche « Science Fiction » et « science populaire »), recherche texte/ISBN/EAN13 dans la barre principale (route via `classify_query` : `isbn` → `Q(isbn_13=v) | Q(isbn_10=v)`, `item` → `items__ean13=v`, sinon FTS5).
...
> Contrainte : la caméra exige un contexte sécurisé (HTTPS). En LAN HTTP, seul
> le repli saisie manuelle fonctionne (HTTPS local box = chantier keebee
> séparé).
...
> - Paramétrage : sections **Impressions — Cartes membres** (`printing_cards` → `card_format`) et **Impressions — Étiquettes codes Ofelia** (`printing_labels` → `item_label_format`) dans `/admin/settings/`. Migration douce depuis l'ancien `label_format` via `_card_settings()` / `_item_label_settings()`.
...
contrôle EAN-13 valide **et** préfixe `290/291/978/979`, **et** confirmée par
...
### 6.12 Catalogage Excel (FEAT-050)
...
**Mode IMPORT** — fichier `.xlsx` avec colonne `ISBN` (+ `LOCATION`/`CATEGORY`
optionnelles) :
...
4. Enrichissement métadonnées **non automatique** (lancer un job FEAT-031
   ensuite si besoin).
