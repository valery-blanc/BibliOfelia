#!/usr/bin/env python3
"""Traductions Sprint 36 — FR → EN/ES/MG.

FEAT-095 : compteur « N membres » au-dessus de la liste filtrée.

À rejouer APRÈS `makemessages` :
    python scripts/translations_sprint36.py
"""
...
PLURALS: dict[str, dict[str, tuple[str, str]]] = {
    "en": {
        "%(counter)s membre": ("%(counter)s member", "%(counter)s members"),
    },
    "es": {
        "%(counter)s membre": ("%(counter)s usuario", "%(counter)s usuarios"),
    },
    "mg": {
        "%(counter)s membre": ("mpampiasa %(counter)s", "mpampiasa %(counter)s"),
    },
}
